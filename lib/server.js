...
function ftpServer(host) {
  ...
  server.on("connection", function (socket) {
    ...
    socket.on("data", function (data) {
      ...
      dotrace("CMD " + command);
      if(!socket.authorized && command !== 'USER' && command !== 'PASS') {
        ...
        switch (command) {
          ...
          case "CWD":
            ...
            var dir = (value.shift() || "").trim();
            if(!dir) {
              socket.writeln("501 CWD failed. Directory needed.");
            } else {
              if(dir.indexOf("/") !== 0) {
                dir = socket.fs.cwd() + dir;
              }
              //socket.writeln("100 CWD Wait...");
              fs.exists(dir, function(exists){
                if(exists) {
                  socket.writeln("250 CWD successful. \"" + socket.fs.chdir(dir) + "\" is current directory");
                } else {
          ...
          case "DELE":
            // Delete file.
            var file = path.normalize(socket.fs.cwd() + value.join(' ').trim());
          ...
          case "PASS":
            ...
            if(!socket.authorized) {
              ...
              bridge.authorize(socket.username, password, function(err){
                if(err) {
          ...
          case "PASV":
            ...
            pasv.on("listening", function () {
              ...
              var i1 = port >> 8;
              var i2 = port % 256;
              socket.writeln("227 Entering Passive Mode (" + socket.pasvhost.replace(".",",") + "," + i1 + "," + i2 + ")");
            });
          ...
          case "PWD":
            ...
            socket.writeln("257 " + socket.fs.cwd() + " is current directory");
            break;
          case "QUIT":
            // Disconnect.
  ...
  //            pasvconn.pipe(ws);
          ...
          case "XPWD":
            //
            socket.writeln("257 " + socket.fs.cwd() + " is the current directory");
            break;
          default:
