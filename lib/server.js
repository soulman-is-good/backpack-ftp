        ...
        switch (command) {
          ...
          case "LIST":
            ...
            socket.datatransfer = function (pasvconn) {
              ...
              dotrace("DATA connect LIST",socket.fs.cwd(), socket.mode);
            ...
            socket.datatransfer = function (pasvconn) {
              ...
              fs.readdir(socket.fs.cwd(), function(err, files) {
                if(err) {
                  ...
                  pasvconn.end(files.join('/r/n'), socket.mode);
                }
              });
            };
            if (!socket.passive) {
          ...
          case "PASV":
            var waitForTransfer = function(psock) {
              if('function' === typeof socket.datatransfer) {
                ...
                setTimeout(function(){waitForTransfer(psock);}, 200);
              }
            };
            // Enter passive mode.
            socket.passive = true;
          ...
          case "TYPE":
            ...
            dotrace("Mode switch to", socket.mode);
            break;
          case "USER":
